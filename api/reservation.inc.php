...
require 'dbh.inc.php';
    ...
    if(empty($fname) || empty($lname) || empty($date) || empty($time) || empty($guests) || empty($tele)) {
     ...
     //Verifica o número maximo de tabelas naquele dia
    ...
    else{$a_tables=5;} //Caso não haja alteração o numero máximo de mesas default é 20
        
        
    //Verifica se a reserva não vai passar o número de mesas maximas no dia
    ...
    if($current_tables + $tables > $a_tables){
        echo "<script> 
        alert('Nós estamos lotados nesta data que você escolheu !! Tente trocar a data ou horário');
         ...
         $sql = "INSERT INTO reservation(f_name, l_name, num_guests, num_tables, rdate, time_zone, telephone, comment, user_fk) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = mysqli_stmt_init($conn);
                 if(!mysqli_stmt_prepare($stmt, $sql)){
                    ...
                    window.location.href = 'reservation.php?reservation=success';
                    </script>";
                    exit();
                }
        }
